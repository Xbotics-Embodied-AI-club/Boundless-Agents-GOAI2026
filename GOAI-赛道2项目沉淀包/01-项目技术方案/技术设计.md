...
3. 在 `project.config.json` 中填写本人注册的小程序 AppID 或官方测试号；占位 AppID 不能用于编译。
...
- **没有 AppID** → 先运行 Web Demo；需要编译小程序时申请自己的小程序 AppID 或官方测试号。
...
3. 使用本人注册的小程序 AppID 或官方测试号编译；占位值不能用于编译。
4. 从首页完整跑通一次学习闭环。
